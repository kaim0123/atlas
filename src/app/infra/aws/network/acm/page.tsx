  ...
  return (
    <DocsPage>
      ...
      <table>
        ...
        <tbody>
          <tr><td className="hl">DNS検証</td><td>指定されたCNAMEレコードをドメインのDNSに追加する</td><td>そのレコードを残しておく限り自動更新され続ける。<Link href="/infra/aws/network/route53">Route 53</Link>を使っていればほぼ自動で設定できる</td></tr>
      ...
      <p>ACMの証明書は、<Link href="/infra/aws/network/infrafront">CloudFront</Link>・ELB(ALB)・API Gatewayなど、AWSの主要なサービスに直接アタッチして使えます。ただし、CloudFrontで使う証明書だけは<Term>us-east-1(バージニア北部)リージョン</Term>で発行されている必要があるという特有の制約があり、CloudFrontを他のリージョンで運用していても証明書だけはus-east-1で発行し直す必要があります。ELBなど他のサービスでは、そのリソースと同じリージョンで発行した証明書を使います。</p>
      ...
      <RelatedNav>
        <RelatedList>
          ...
          <RelatedLink href="/infra/aws/network/route53" tag="AWS">Route 53</RelatedLink>
