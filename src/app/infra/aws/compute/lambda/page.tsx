    ...
    <DocsPage>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td className="hl">非同期呼び出し</td><td>S3・<Link href="/infra/aws/integration/sns">SNS</Link></td><td>呼び出し元は結果を待たず、失敗時はLambda側で自動リトライされる</td></tr>
          <tr><td className="hl">ポーリング呼び出し</td><td><Link href="/infra/aws/integration/sqs">SQS</Link>・DynamoDB Streams</td><td>Lambdaのサービス側が定期的にキューやストリームを監視し、たまったデータをまとめて処理する</td></tr>
        </tbody>
      </table>
      ...
      <p>Lambdaが返す・受け取るイベントの多くは<Link href="/infra/aws/integration">アプリケーション統合</Link>で見たSQS・SNS・EventBridge経由でやり取りされます。</p>
