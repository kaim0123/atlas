...
export default function Page() {
  return (
    <DocsPage>
      ...
      <p><Term>EC2(Elastic Compute Cloud)</Term>は、AWSのデータセンター上にある仮想マシンを1台単位で借りられるサービスです。<Link href="/computer/os">OSの仕組み</Link>ページで見た「OSがハードウェアを抽象化する」という発想をもう一段進め、ハードウェアそのものを仮想化して切り売りしています。OS・ミドルウェア・アプリケーションはすべて自分で選び、インストールし、管理します。</p>
      ...
      <p><Term>Lambda</Term>はコードの断片(関数)をアップロードしておくと、イベント(HTTPリクエスト・ファイルアップロード・スケジュールなど)が起きたときだけ実行環境が自動的に用意され、処理が終われば消える仕組みです。この形態を<Term>サーバーレス</Term>と呼びます。EC2のようにサーバーを起動・停止・パッチ適用する作業自体が発生せず、実行された時間とリクエスト数に応じてのみ課金されます。呼び出し方やコールドスタートなど実行モデルの詳細は<Link href="/infra/aws/compute/lambda">Lambdaのページ</Link>で扱います。</p>
      ...
      <p>コンピューティングで処理した結果は、どこかに保存しなければ消えてしまいます。次のページでは、その保存先を選ぶ「<Link href="/infra/aws/storage">ストレージ</Link>」を見ていきます。</p>
