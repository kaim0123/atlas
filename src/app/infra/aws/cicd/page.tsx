  ...
  return (
    <DocsPage>
      <Hero>
        ...
        <Lead>
          <Link href="/infra/aws/container">コンテナ</Link>ページで見た「箱」も、その中身であるコードも、人手でビルドしてサーバーへ配置していては、変更のたびに時間がかかり、手順のミスも起こります。<Term>CI/CD(継続的インテグレーション/継続的デリバリー)</Term>は、コードのビルド・テスト・デプロイを自動化し、変更を安全かつ高頻度に本番へ届けるための仕組みです。AWSでは<Term>CodeBuild</Term>がビルドを、<Term>CodePipeline</Term>がその一連の流れの調整を担います。
        </Lead>
      </Hero>
      ...
      <p>コードを自動でビルド・デプロイできるようになったら、最後は分かれたサービス同士をどうつなぐかという「<Link href="/infra/aws/integration">アプリケーション統合</Link>」です。まずは実行エンジンである<Link href="/infra/aws/cicd/codebuild">CodeBuild</Link>から見ていきます。</p>

      <IndexGrid>
        <IndexCard href="/infra/aws/cicd/codebuild" num="01" title="CodeBuild">
          buildspecに従ってソースをビルド・テストする、マネージドなビルド環境。
        </IndexCard>
        <IndexCard href="/infra/aws/cicd/codepipeline" num="02" title="CodePipeline">
