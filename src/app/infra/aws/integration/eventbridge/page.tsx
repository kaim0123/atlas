  ...
  return (
    <DocsPage>
      <Hero>
        ...
        <Lead>
          <Term>EventBridge</Term>は、発生したイベントの<Term>内容そのもの</Term>を見て、あらかじめ定義したルールに一致するものだけを該当する宛先へ自動的に振り分ける<Term>イベントバス</Term>サービスです。<Link href="/infra/aws/integration/sns">SNS</Link>が「トピックを購読しているか」だけで配信先を決めるのに対し、EventBridgeは「イベントの中身がどんな条件を満たすか」で振り分け先を決められます。
        </Lead>
      </Hero>
      ...
      <p>これで、<Link href="/infra/aws/basics">AWSの基礎</Link>から<Link href="/infra/aws/integration">アプリケーション統合</Link>まで、AWSの中心となる10分野を一通り見たことになります。実際のシステムでは、ここで見た個々のサービスを組み合わせて、<Link href="/design/architecture">アーキテクチャ</Link>ページで見た構成として組み上がります。残る最後の1分野、<Link href="/infra/aws/iac">IaC</Link>では、それらの組み合わせを実際にコードでどう定義するかを見ていきます。</p>
