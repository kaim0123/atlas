  ...
  return (
    <DocsPage>
      <Hero>
        ...
        <Lead>
          <Term>SNS(Simple Notification Service)</Term>は、1件のメッセージを<Term>トピック</Term>に発行するだけで、そのトピックを購読している複数の宛先へ同時に配信する<Term>Pub/Sub(パブリッシュ/サブスクライブ)</Term>型のサービスです。<Link href="/infra/aws/integration/sqs">SQS</Link>が「1対1」で受信側を1つ想定するのに対し、SNSは「1対多」の配信を前提にしています。
        </Lead>
      </Hero>
      ...
      <p>最も代表的な使い方が<Term>ファンアウトパターン</Term>です。1件のイベント(例: 「注文が確定した」)をSNSトピックに発行すると、在庫更新用・請求処理用・通知メール用など複数のSQSキューへ同時に複製されます。各キューの受信側は他の受信側の処理状況を知らずに、自分の担当分だけを<Link href="/infra/aws/integration/sqs">SQS</Link>のPull型キューから安全に取り出して処理できます。SNS単体でも複数宛先へ配信できますが、SQSを間に挟むことで、受信側が一時的に処理できない状態でもメッセージを取りこぼさずに済みます。</p>
      ...
      <MarkNote>→ SNSの配信は「購読しているかどうか」だけが基準です。メッセージの内容そのものに応じて振り分け先を柔軟に変えたい場合は、次に見る<Link href="/infra/aws/integration/eventbridge">EventBridge</Link>のルールベースのルーティングがより向いています。</MarkNote>

      <RelatedNav>
        <RelatedList>
          <RelatedLink href="/infra/aws/integration/sqs" tag="AWS">SQS</RelatedLink>
          <RelatedLink href="/infra/aws/integration/eventbridge" tag="AWS">EventBridge</RelatedLink>
