...
export default function Page() {
  return (
    <DocsPage>
      ...
      <p><Term>S3(Simple Storage Service)</Term>は、ファイルを「オブジェクト」という単位のまま、事実上無制限の容量で保存できるサービスです。OSのファイルシステムのようにディレクトリ階層でマウントするのではなく、<Term>バケット</Term>という入れ物にHTTP経由でオブジェクトを出し入れします。画像・動画・ログ・バックアップなど、「そのまま保存して、そのまま取り出す」用途に向いています。ストレージクラスやアクセス制御の詳細は<Link href="/infra/aws/storage/s3">S3のページ</Link>で扱います。</p>
      ...
      <p><Term>EBS(Elastic Block Store)</Term>は、<Link href="/infra/aws/compute">EC2</Link>インスタンスに取り付ける「仮想ハードディスク」です。OSからは通常のディスクとして認識され、ファイルシステムを作ってフォルダ階層で自由に読み書きできます。EC2インスタンス本体を削除してもEBSボリュームだけを残すこともでき、インスタンスの入れ替えとデータの寿命を切り離せます。</p>
      ...
      <p>保存したデータをどう検索・集計するかは、次のページで見る「<Link href="/infra/aws/database">データベース</Link>」の役割です。一方、これらのストレージやサーバーを外部からどう到達可能にするかは「<Link href="/infra/aws/network">ネットワーキング</Link>」が扱います。</p>
