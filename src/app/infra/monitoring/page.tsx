...
export default function Page() {
  return (
    <DocsPage>
      ...
      <p><Link href="/ops/monitoring">監視・保守</Link>では、アプリケーションの<Term>メトリクス・ログ・トレース</Term>を扱いました。このページでは視点を切り替え、サーバーやネットワーク機器そのものの死活・リソースを見る<Term>インフラ視点の監視</Term>を扱います。アプリのコードが1行も悪くなくても、土台側の異常でサービスが止まることは珍しくありません。</p>
      ...
      <p>機器やサーバーから集めたログの中から、エラーやハードウェア異常を示すパターン(「disk error」「link down」など)を検索して検知するのが<Term>ログ監視</Term>です。<Link href="/ops/monitoring">監視・保守</Link>で扱ったアラート設計(アラート疲れを避ける、通知先を絞る、といった全体設計)はそのままインフラ側にも当てはまるため、ここでは「インフラのどの指標に、どんなしきい値を置くか」に絞って整理します。</p>
      ...
      <DocsFooter>Atlas &middot; インフラ &middot; インフラの監視</DocsFooter>
    </DocsPage>
  );
}
