  ...
  return (
    <DocsPage>
      <Hero>
        ...
        <h1>ネットワーク機器の役割分担 ― 何が何をしているか</h1>
        <Lead>
          家庭やオフィスには、ONU・ルーター・スイッチ・アクセスポイントなど、似たような小さな箱がいくつも並んでいます。どれも黒くて光る箱で見た目はそっくりですが、担当している仕事はまったく別です。ここでは実機として、それぞれがネットワークのどこに配置され、何をしているのかを1つずつ整理します。
        </Lead>
      </Hero>

      <p>「<Link href="/network/protocols">通信プロトコル</Link>」では、通信の「ルール」が物理層からアプリケーション層まで階層になっていることを見ました。実は目の前にある機器たちも、この層のどこを見て動いているかによって、はっきり役割が分かれています。</p>

      <Heading num="01">回線の入り口 ― ONUとモデム</Heading>
      ...
      <Heading num="02">同じ箱に見えて中身が違う ― ハブ・L2スイッチ・L3スイッチ・ルーター</Heading>
      ...
      <Heading num="03">無線とPoE ― アクセスポイントとPoEスイッチ</Heading>
      ...
      <Heading num="04">ネットワークの境界に立つ機器 ― ファイアウォール・ロードバランサ・VPN装置</Heading>
      ...
      <p><Term>ファイアウォール</Term>は、社内ネットワークとインターネットの境目など、ネットワークの出入り口に置かれ、通過してよい通信とそうでない通信を判断する機器です。ルーターに機能として内蔵されていることもあれば、専用の機器(アプライアンス)として別に設置されることもあります。検査の中身がどう違うか(パケットフィルタ型・ステートフルインスペクション型・WAFなど)は「<Link href="/network/protocols">通信プロトコル</Link>」で扱った通りで、ここで大事なのは<strong>ネットワークの境界という「場所」に立つ機器だ</strong>という位置づけです。</p>
      ...
      <DocsFooter>Atlas &middot; ネットワーク &middot; ネットワーク機器の役割</DocsFooter>
    </DocsPage>
  );
}
