  ...
  return (
    <DocsPage>
      <Hero>
        ...
        <h1>概念設計と正規化 ― エンティティ抽出からテーブルへ</h1>
        <Lead>
          前のページ「<Link href="/dev/database">データベース</Link>」で見た<Term>概念スキーマ</Term>は、どうやって作ればよいのでしょうか。業務要件という文章から、エンティティを抽出し、ER図で関連を可視化し、<Term>正規化</Term>という手順でテーブルを段階的に分割していく ― この一連の流れを、「顧客が商品を注文する」という通しの例で見ていきます。
        </Lead>
      </Hero>
      ...
      <p>ここまでで、要件から論理的なテーブル設計(概念スキーマ)を導く手順が固まりました。次のページ「<Link href="/dev/database/physical">物理設計と運用</Link>」では、この設計を実際のテーブル定義・インデックスとしてDBMS上に実体化し、可用性を保つための冗長化・バックアップまで見ていきます。</p>
      ...
      <DocsFooter>Atlas &middot; 開発 &middot; データベース &middot; 概念設計と正規化</DocsFooter>
    </DocsPage>
  );
}
