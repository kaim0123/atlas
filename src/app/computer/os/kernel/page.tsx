...
export default function Page() {
  return (
    <DocsPage>
      ...
      <p><Link href="/computer/os">OSの仕組み</Link>のページでは、OSがハードウェアとアプリの間で「調整役」を務めること、そしてその中核にカーネルがあり、コンピュータが4層構造で積み上がっていることを俯瞰しました。このページでは、その<strong>カーネルそのもの</strong>にズームインします ― どんな役割を担い、どんな設計思想があり、なぜ「特権」という概念が必要なのかを見ていきます。</p>
      ...
      <p>5番目の<Term>システムコール</Term>は、アプリが直接ハードウェアを触れない代わりに「カーネルさん、これをやってください」と依頼するための仕組みです。これ自体が大きなテーマなので、詳しくは<Link href="/computer/os/syscall">システムコール</Link>のページで扱います。</p>
      ...
      <p>ユーザーモードからカーネルモードへは<strong>勝手には入れません</strong>。特別な手続き ― その正式な窓口が<Link href="/computer/os/syscall">システムコール</Link>です。アプリが「これやってください」と礼儀正しく依頼する仕組みになっています。カーネルモードでは、次のような追加の特権が使えます。</p>
      <ul>
      ...
      <p>ここではカーネルが「何を管理し、どう設計され、どんな特権を持つか」という骨格を見ました。次は、その管理対象の筆頭である<Link href="/computer/os/process">プロセスとスレッド</Link>を掘り下げ、その後アプリとカーネルの架け橋である<Link href="/computer/os/syscall">システムコール</Link>の内部動作へと進みます。</p>
      ...
      <DocsFooter>Atlas &middot; コンピュータ &middot; カーネルの役割と設計</DocsFooter>
    </DocsPage>
  );
}
