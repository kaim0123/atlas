...
export default function Page() {
  return (
    <DocsPage>
      ...
      <p>各プロセスに割り当てる時間を<Term>タイムクオンタム(タイムスライス)</Term>と呼び、時間切れで強制的に次へ切り替えることを<Term>プリエンプション(先取り)</Term>と呼びます。Linuxの<Term>CFS(Completely Fair Scheduler)</Term>は、<strong>仮想実行時間</strong>が最も少ないプロセスを次に実行し、全プロセスの実行時間を均等に保ちます。</p>
